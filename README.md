...
7. Enable & run the systemd unit for wpa_supplicant. _*You can change the network interface name to your own._
```
systemctl enable wpa_supplicant@wlp1s0.service&& systemctl start wpa_supplicant@wlp1s0.service
```
8. `nano /etc/network/interfaces` and edit the file as below:
...
```
9. Disable & reenable the wlp1s0 wifi interface. Disable the autogenerated vmbr0 interface & enable the new vmbr1.
```
ifdown wlp1s0 && ifdown vmbr0
ifup wlp1s0 && ifup vmbr1
```
10. Now you should be able to connect to the internet. Test `ping 8.8.8.8`
11. Install dnsmasq & dnsutils `apt install dnsmasq dnsutils -y`
12. Run `nano /etc/dnsmasq.conf`, at the bottom line add the following line:
```
domain=pv.local
interface=vmbr1
